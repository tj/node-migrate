...
function migrate (set, direction, migrationName, fn) {
  ...
  function next (migration) {
    ...
    var migrationPromise = migration[direction](function (err) {
      if (migrationCompleted) return fn(new Error('Migration was already run. Please provide callback OR promise, not both.'))
      if (err) return fn(err)
      completeMigration()
    })

    if (typeof Promise !== 'undefined' && migrationPromise instanceof Promise) {
      migrationPromise
        .then(function () {
          if (migrationCompleted) return fn(new Error('Migration was already run. Please provide callback OR promise, not both.'))
          completeMigration()
