...
/**
 * Calls the migrations/hooks.js's before_load() if fn exists.
 *
 */
Set.prototype.loadWithHooks = function(fn) {
  var self = this;

  if(this.hooks.before_load){
    this.hooks.before_load(this.path, function(){
      self.load(fn);
    });
  }else{
    this.load(fn);
...
Set.prototype.afterSave = function(json, fn) {
  var self = this;

  if(this.hooks.after_save){
    this.hooks.after_save(json, function(){
      fn();
    });
  }else{
    fn();
...
Set.prototype.migrate = function(direction, fn, migrationName){
  var self = this;
  fn = fn || function(){};
  this.loadWithHooks(function(err, obj){
    if (err) {
