...
var GenerateHash = require('./generate-hash')
  ...
  store.load(function (err, state) {
    ...
    fs.readdir(migrationsDirectory, function (err, files) {
      ...
      var migrations = files.map(function (file) {
        ...
        var hash = GenerateHash(migrationsDirectory + file, 'sha1')
        var migration = new Migration(file, mod.up, mod.down, mod.description, hash)
      ...
      state.migrations && state.migrations.forEach(function (m) {
        ...
        } else if (m.hash !== null && m.hash !== migMap[m.title].hash) {
          fn(new Error('CheckSum is wrong for migration file: ' + m.title))
      ...
      // initial not tested version. find first not runned migration. Then if there is any run migration (later) - error
      var iter = -1
      var firstNotRun = -1
      var isThereAnyNextRunnedMigration = false
      migrations.forEach(function(mig) {
        iter++
        if (mig.timestamp == null) {
          if (firstNotRun !== -1 && iter > firstNotRun) {
            isThereAnyNextRunnedMigration = true
          }
          firstNotRun = iter
        }
        if (isThereAnyNextRunnedMigration)
        {
          fn(new Error('The history is not continuous. Please check: ' + mig.title))
        }
      })
